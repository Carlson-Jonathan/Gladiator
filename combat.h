...
void calculateDamageTypes(const Character & aggressor, short damage[]) {
   ...
   damage[0] = baseDamage * aggressor.weapon->crush;
   damage[1] = baseDamage * aggressor.weapon->chop;
   damage[2] = baseDamage * aggressor.weapon->slash;
   damage[3] = baseDamage * aggressor.weapon->stab;
}
...
void calculateDamageBlock(const Character & victim, short block[]) {
   ...
   block[0] = armor.defencePower * armor.crush;
   block[1] = armor.defencePower * armor.chop;
   block[2] = armor.defencePower * armor.slash;
   block[3] = armor.defencePower * armor.stab;
}
...
void focus(Character & character) {
   ...
   cout << "\t" << character.name << character.name 
        << " raises their guard to plan their next strike.\n" << endl;
